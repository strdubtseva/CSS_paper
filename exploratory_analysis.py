...
# Create word cloud for comments in each subreddit
for subreddit in comments_data["subreddit"].unique():
...
for subreddit in tfidf_df.index:
    top_words = tfidf_df.loc[subreddit].sort_values(ascending=False).head(20)
